    ...
    $.post(get_all_plans, params,
        function(response) {
            ...
            if (app.plans.length > 0) {
                app.selected = app.plans[app.plans.length - 1].name;;
            }
        }
    );
...
var handlePostDelete = function(userPlanID) {
    $.post (delete_post, { id: userPlanID}, function(response) {
    window.location.reload();
    })
};
...
var handlesavePlan = function() {
    ...
    var newPlan = {
        planname: "Plan " + (app.plans.length + 1),
